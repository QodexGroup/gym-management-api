...
- **All customers can check in regardless of membership status, plan, or expiry.** This includes active members, expired members, guests, day-use visitors, and non-members.
- A customer can only be checked in once per day per account (duplicate guard).
- Check-out only transitions from `INSIDE` to `OUTSIDE`.

> **Future:** An account-level setting may be added to restrict check-ins to active members only. For now, the walk-in flow is open to everyone.
...
> **No membership restriction:** Any customer can be checked in regardless of their membership status (active, expired, no plan, guest, etc.).
...
> **No membership restriction:** Any customer can check in via QR regardless of their membership status (active, expired, no plan, guest, etc.).
...
*Version: 1.1 — Open check-in policy: all customers allowed regardless of membership status*
